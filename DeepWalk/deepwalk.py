...
def deepwalk(format,input,max_memory_data_size,number_walks,representation_size,walk_length, window_size,workers,output,undirected=True,seed=0,vertex_freq_degree=False):
  graph=Graph()
  if format == "adjlist":
    G = load_adjacencylist(input, undirected=undirected)
  elif format == "edgelist":
    G = load_edgelist(input, undirected=undirected)
  elif format == "mat":
    G = load_matfile(input, variable_name=matfile_variable_name, undirected=undirected)
  else:
  ...
  if data_size < max_memory_data_size:
    ...
    walk_files = write_walks_to_disk(G, walks_filebase, num_paths=number_walks,
                                         path_length=walk_length, alpha=0, rand=random.Random(seed),
                                         num_workers=workers)
